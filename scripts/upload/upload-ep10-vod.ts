#!/usr/bin/env npx tsx
/**
 * 10화 VOD 업로드 스크립트 (용병데이)
 *
 * 소스 파일 1개 (99GB, 13h29m) → FFmpeg -c copy로 3시간 단위 분할 → 5파트
 *   Part 1: 0h ~ 3h (~22GB)
 *   Part 2: 3h ~ 6h (~22GB)
 *   Part 3: 6h ~ 9h (~22GB)
 *   Part 4: 9h ~ 12h (~22GB)
 *   Part 5: 12h ~ 13h29m (~11GB)
 *
 * 사용법:
 *   npx tsx scripts/upload-ep10-vod.ts --dry-run     # 분할 계획 확인
 *   npx tsx scripts/upload-ep10-vod.ts --split-only   # 분할만 실행 (확인용)
 *   npx tsx scripts/upload-ep10-vod.ts --skip-split   # 분할 스킵, 업로드만 (이미 분할된 경우)
 *   npx tsx scripts/upload-ep10-vod.ts                # 전체 실행 (분할+업로드+임시삭제)
 *   npx tsx scripts/upload-ep10-vod.ts --keep-files   # 업로드 후 임시파일 유지
 *   npx tsx scripts/upload-ep10-vod.ts --start-part 3 # 3번 파트부터 재개
 */
...
const TEMP_DIR = path.join(os.tmpdir(), 'rg-vod-ep10')

// 10화 소스 파일 경로
const SOURCE_FILE = '/Users/bagjaeseog/Downloads/2026-02-10 13-54-11.mp4'

const BASE_TITLE = '엑셀부 시즌1_10화 용병데이'
const TOTAL_PARTS = 5
...
function calculateParts(totalDuration: number): PartInfo[] {
  ...
  while (currentTime < totalDuration) {
    ...
    parts.push({
      ...
      uploadPath: path.join(TEMP_DIR, `ep10_part${partNumber}.mp4`),
    })
...
async function main() {
  ...
  try {
    ...
    log('⚠️', '길이 확인 실패 - 예상값(48559s = 13h29m) 사용')
    totalDuration = 48559
  }
  ...
  if (splitOnly) {
    ...
    console.log(`\n   ✅ 확인 후 업로드: npx tsx scripts/upload-ep10-vod.ts --skip-split`)
  } else {
    console.log('📊 최종 결과')
    console.log('═'.repeat(60))
