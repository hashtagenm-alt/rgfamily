#!/usr/bin/env npx tsx
/**
 * 15화 VOD 업로드 스크립트 (최종직급전)
 *
 * 소스 파일 1개 (89GB) → FFmpeg -c copy로 3시간 단위 분할 → 파트별 업로드
 *
 * 사용법:
 *   npx tsx scripts/upload-ep15-vod.ts --dry-run     # 분할 계획 확인
 *   npx tsx scripts/upload-ep15-vod.ts --split-only   # 분할만 실행 (확인용)
 *   npx tsx scripts/upload-ep15-vod.ts --skip-split   # 분할 스킵, 업로드만 (이미 분할된 경우)
 *   npx tsx scripts/upload-ep15-vod.ts                # 전체 실행 (분할+업로드+임시삭제)
 *   npx tsx scripts/upload-ep15-vod.ts --keep-files   # 업로드 후 임시파일 유지
 *   npx tsx scripts/upload-ep15-vod.ts --start-part 3 # 3번 파트부터 재개
 */
...
const TEMP_DIR = path.join(os.tmpdir(), 'rg-vod-ep15')

// 15화 소스 파일 경로
const SOURCE_FILE = '/Users/bagjaeseog/Downloads/15화 최종직급전 [RG패밀리 엑셀부 시즌1].mp4'

const BASE_TITLE = '엑셀부 시즌1_15화 최종직급전'
...
function calculateParts(totalDuration: number): PartInfo[] {
  ...
  while (currentTime < totalDuration) {
    ...
    parts.push({
      ...
      uploadPath: path.join(TEMP_DIR, `ep15_part${partNumber}.mp4`),
    })
...
async function main() {
  ...
  try {
    ...
    log('⚠️', '길이 확인 실패 - 예상값(43200s = 12h) 사용')
    totalDuration = 43200
  }
  ...
  if (splitOnly) {
    ...
    console.log(`\n   ✅ 확인 후 업로드: npx tsx scripts/upload-ep15-vod.ts --skip-split`)
  } else {
    console.log('📊 최종 결과')
    console.log('═'.repeat(60))
