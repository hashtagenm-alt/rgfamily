#!/usr/bin/env npx tsx
/**
 * 12화 VOD 업로드 스크립트 (주차방지데이)
 *
 * 소스 파일 1개 (91GB) → FFmpeg -c copy로 3시간 단위 분할 → 4~5파트
 *
 * 사용법:
 *   npx tsx scripts/upload-ep12-vod.ts --dry-run     # 분할 계획 확인
 *   npx tsx scripts/upload-ep12-vod.ts --split-only   # 분할만 실행 (확인용)
 *   npx tsx scripts/upload-ep12-vod.ts --skip-split   # 분할 스킵, 업로드만 (이미 분할된 경우)
 *   npx tsx scripts/upload-ep12-vod.ts                # 전체 실행 (분할+업로드+임시삭제)
 *   npx tsx scripts/upload-ep12-vod.ts --keep-files   # 업로드 후 임시파일 유지
 *   npx tsx scripts/upload-ep12-vod.ts --start-part 3 # 3번 파트부터 재개
 */
...
const TEMP_DIR = path.join(os.tmpdir(), 'rg-vod-ep12')

// 12화 소스 파일 경로
const SOURCE_FILE = '/Users/bagjaeseog/Downloads/엑셀부 시즌1_12화 주차방지데이.mp4'

const BASE_TITLE = '엑셀부 시즌1_12화 주차방지데이'
...
function calculateParts(totalDuration: number): PartInfo[] {
  ...
  while (currentTime < totalDuration) {
    ...
    parts.push({
      ...
      uploadPath: path.join(TEMP_DIR, `ep12_part${partNumber}.mp4`),
    })
...
async function main() {
  ...
  console.log('🎬 12화 VOD 업로드 (주차방지데이)')
  console.log('═'.repeat(60))
  ...
  try {
    ...
    log('⚠️', '길이 확인 실패 - 예상값(44100s = 12h15m) 사용')
    totalDuration = 44100
  }
  ...
  if (splitOnly) {
    ...
    console.log(`\n   ✅ 확인 후 업로드: npx tsx scripts/upload-ep12-vod.ts --skip-split`)
  } else {
    console.log('📊 최종 결과')
    console.log('═'.repeat(60))
