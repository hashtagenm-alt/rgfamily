#!/usr/bin/env npx tsx
/**
 * 8화 VOD 업로드 스크립트 (뉴시그데이)
 *
 * 소스 파일 2개 → 최종 5파트로 Cloudflare Stream 업로드 → DB 저장
 *
 * 파일1 (10GB, 1h25m) → 그대로 업로드 → Part 1
 * 파일2 (86GB, 11h43m) → FFmpeg -c copy로 3시간 단위 분할 → Part 2~5
 *   Part 2: 0h ~ 3h (~22GB)
 *   Part 3: 3h ~ 6h (~22GB)
 *   Part 4: 6h ~ 9h (~22GB)
 *   Part 5: 9h ~ 11h43m (~20GB)
 *
 * 사용법:
 *   npx tsx scripts/upload-ep8-vod.ts --dry-run     # 분할 계획 확인
 *   npx tsx scripts/upload-ep8-vod.ts                # 전체 실행
 *   npx tsx scripts/upload-ep8-vod.ts --skip-split   # 분할 스킵 (이미 완료된 경우)
 *   npx tsx scripts/upload-ep8-vod.ts --start-part 3 # 3번 파트부터 재개
 */
...
const TEMP_DIR = path.join(os.tmpdir(), 'rg-vod-ep8')

// 8화 소스 파일 경로
const SOURCE_DIR = '/Volumes/Untitled/08화 뉴시그데이'
const SOURCE_FILES = {
  file1: '엑셀부 시즌1_08화 뉴시그데이 1.mp4',  // 10GB, 1h25m → 그대로
  file2: '엑셀부 시즌1_08화 뉴시그데이 2.mp4',  // 86GB, 11h43m → 4파트 분할
}

const BASE_TITLE = '엑셀부 시즌1_08화 뉴시그데이'
const TOTAL_PARTS = 5
...
function calculateParts(): PartInfo[] {
  ...
  while (currentTime < file2Duration) {
    ...
    parts.push({
      ...
      uploadPath: path.join(TEMP_DIR, `ep8_part${partNumber}.mp4`),
      isTempFile: true,
    })
...
async function main() {
  ...
  console.log('🎬 8화 VOD 업로드 (뉴시그데이) - 5파트')
  console.log('═'.repeat(60))
