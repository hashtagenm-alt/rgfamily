...
export async function refreshSeasonRankings(
  ...
  return adminAction(
    async (supabase) => {
      // 1. donations에서 해당 시즌 + unit 데이터 가져오기 (페이지네이션)
...
export async function importDonationsCsv(
  ...
  return adminAction(
    async (supabase) => {
      ...
      // 2. 에피소드 존재 확인 (unit 포함)
      const { data: episode, error: epError } = await supabase
        .from('episodes')
        .select('id, season_id, unit')
        .eq('id', episodeId)
        .single()
      ...
      for (let i = 0; i < rows.length; i += batchSize) {
        const batch = rows.slice(i, i + batchSize).map((r) => ({
          ...
          unit: (episode.unit as 'excel' | 'crew') || 'excel',
        }))

        const { error: insertError } = await supabase.from('donations').insert(batch)
        if (insertError)
          throw new Error(
            `데이터 삽입 실패 (batch ${Math.floor(i / batchSize) + 1}): ${insertError.message}`
          )
      ...
      if (updateError) {
        logger.dbError('update', 'episodes', updateError, {
          context: { note: '데이터는 임포트됨' },
        })
      }
...
> {
  return adminAction(async (supabase) => {
    let query = supabase
      .from('episodes')
      .select('id, episode_number, title, is_finalized, unit')
      .eq('season_id', seasonId)
