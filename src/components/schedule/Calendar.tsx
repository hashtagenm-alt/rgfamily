...
import { Group, Stack, Text, ActionIcon, SegmentedControl, Loader } from '@mantine/core'
...
export default function Calendar() {
  ...
  return (
    <div className="flex w-full justify-center px-4 pb-16 md:px-8">
      <Stack gap="lg" className="w-full max-w-[1100px]">
        ...
        <Stack gap="md">
          ...
          <Group gap="lg" align="center" justify="center">
            <ActionIcon
              ...
              size="xl"
              radius="xl"
              className="border-(--card-border)] bg-(--surface)] text-(--text-muted)] hover:bg-(--surface-hover)] hover:text-(--text-primary)] hover:border-(--card-border-hover)] border transition-all"
            >
              <ChevronLeft size={24} />
            </ActionIcon>
            ...
            >
              <Text
                size="xl"
                fw={800}
                className="text-(--text-primary)] min-w-[140px] text-center text-lg tracking-wide sm:min-w-[200px] sm:text-2xl"
              >
                {monthYear}
              </Text>
            </motion.div>
        ...
        <div className="border-(--card-border)] bg-(--card-bg)] overflow-hidden rounded-2xl border shadow-sm">
          {/* Weekday Labels */}
          <div className="bg-(--divider)] grid grid-cols-7 gap-0.5">
            {WEEKDAYS.map((day, index) => (
              <div
                key={day}
                className={`bg-(--surface)] px-1 py-2 text-center text-xs font-semibold tracking-wider uppercase sm:px-3 sm:py-4 sm:text-sm ${
                  index === 0
                    ? 'text-[#ef4444]'
                    : index === 6
                      ? 'text-(--text-secondary)]'
