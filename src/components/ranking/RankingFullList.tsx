...
const TIER_BREAKPOINTS = new Set([3, 10])
      ...
      {displayRankings.map((item, index) => {
        ...
        const Content = (
          ...
          >
            ...
            <div className={styles.progressBar} style={{ width: `${scorePercent}%` }} />
            ...
            <div className={styles.scoreColumn}>
              <span className={styles.viewerScore}>{item.viewerScore.toLocaleString()}</span>
            ...
            <div className={styles.countColumn}>
              {item.donationCount > 0 ? (
                <span className={styles.donationCount}>{item.donationCount}회</span>
              ) : (
        ...
        // 티어 구분선 + 아이템 래핑
        const showDivider = TIER_BREAKPOINTS.has(item.rank - 1) && item.rank > 1
        ...
        if (showDivider) {
          return (
            <div key={`${item.donorName}-${index}`}>
              ...
              {hasVipPage ? (
                <Link href={`/ranking/vip/${item.donorId}`} className={styles.vipLink}>
                  {Content}
                </Link>
              ) : (
                <div className={styles.noLink}>{Content}</div>
              )}
            </div>
          )
        }
